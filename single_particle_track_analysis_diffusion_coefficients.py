# -*- coding: utf-8 -*-
"""
Estimation of diffusion parameters from power-law scaling (MSD ~ lag time^α) by computing the time-averaged mean squared 
displacement (MSD) for each track.

Code includes estimation of drift
Remove of static error
Change criterion σ/Δt < 1 
...
taus = [0.01] #min time displacement for MSD calculation 
...
error_file = 'D:082124_082524 folder for not using files in coding/GEM fixed 35oC repeat/MSD_lagtime5.0ms repeat.csv'
...
experiments = [f for f in os.listdir(ExperDirectory) if 'repeat' not in f.lower()]
...
for experiment in experiments: 
    ...
    for tau in taus:
        ...
        for min_trajectory_length in min_trajectory_lengths:
            ...
            directory = f'ARTICLE_V3_{experiment_condition}_R2{r2}_{min_trajectory_length}trlength_5Derr_5aerr_lagtime{tau*1000}ms_nw{nw:.5f}T{T} double check'
