...
taus = [0.005] #time displacement for MSD calculation 
...
def FitMSDerrorFunction(name, x, y, points=10, R2=0.9, yerr = None):
    te = 0.005
    ...
    params, covariance = curve_fit(MSD_LocalisationError_functionV2, x[0:points], y[0:points], sigma = yerr[0:points])
...
def FitLinearFunction(name, x, y, points=10, R2=0.9, yerr = None):
    ...
    # Fit the linear function to the data with uncertainties
    params, covariance = curve_fit(linear_function, x[0:points], y[0:points], p0 =[1, 0.3], maxfev=2000, sigma = yerr[0:points])
...
for experiment in experiments: 
    ...
    for tau in taus:
        ...
        for min_trajectory_length in min_trajectory_lengths:
            ...
            directory = f'ARTICLE_V3_{experiment_condition}_R2{r2}_{min_trajectory_length}trlength_5Derr_5aerr_lagtime{tau*1000}ms_nw{nw:.5f}T{T} DYNAMIC AND STATIC ERROR'
            ...
            for file in files: 
                ...
                with open(f'{parent_dir}{file}/{experiment_condition}/converted_data_length10.csv') as data:
                    ...
                    if save == True: 
                        plt.savefig(f'{parent_dir}{directory}/x_displacement_{file}.pdf', dpi=300)
                        plt.clf()
                    plt.figure(4)
                    ...
                    if save == True: 
                        plt.savefig(f'{parent_dir}{directory}/y_displacement_{file}.pdf', dpi=300)
                        plt.clf()
                    ...
                    for trajectory in trajectories_interpolated:
                        ...
                        #plt.figure()
                        #plt.errorbar(time_data, msd_data, yerr=msd_std_data)
                        ...
                        MSD_limit = (int(round((len(time_data)-1)/2)))
                        
                        for p in range(3, MSD_limit):
                            try:
                                result = FitMSDerrorFunction(name, time_data[0:MSD_limit], msd_data[0:MSD_limit], points=p, R2=r2, yerr = msd_std_data[0:MSD_limit])
                                if result[5] < r2:
                                    break
                            except RuntimeError:
                                result = None
                        
                        if p>3 and result != None and result[5] > r2:
                            ...
                            #plt.xlabel(f'R2 = {result[5]:.2f}, {Da_value}, {a_value}')
                            #plt.show()
                            ...
                            if Da_err*5<=Da_value and a_err*5<a_value: 
                                ...
                                #plt.errorbar(time_data, msd_data, yerr=msd_std_data)
                                #plt.plot(result[6], result[7])
                                #plt.xlabel(f'R2 = {result[5]:.2f}, {Da_value}, {a_value}, passed')
            ...
            plt.xticks()
            plt.yticks()
            plt.xlabel(f'Da values, <Da> = {np.mean(Diffusion_AllCells):.2f} ± {np.std(Diffusion_AllCells):.2f}, median = {np.median(Diffusion_AllCells):.2f}')
            plt.ylabel('Probability Density')
            plt.title('Histogram and PDF of Da')
            plt.legend()
            if save == True: 
                plt.savefig(f'{parent_dir}{directory}/Histogram of Da.pdf', dpi=300)
            plt.show()
            
            pdf_diffusion = PDF(Deff_AllCells)
            ...
            plt.xticks()
            plt.yticks()
            plt.xlabel(f'Deff values, <Deff> = {np.mean(Deff_AllCells):.2f} ± {np.std(Deff_AllCells):.2f}, median = {np.median(Deff_AllCells):.2f}')
            plt.ylabel('Probability Density')
            plt.title('Histogram and PDF of Deff')
            plt.legend()
            if save == True: 
                plt.savefig(f'{parent_dir}{directory}/Histogram of Deff.pdf', dpi=300)
            plt.show()
            
            pdf_diffusion = PDF(delta_AllCells)
            ...
            plt.xticks()
            plt.yticks()
            plt.xlabel(f'Da values, <delta> = {np.mean(delta_AllCells):.2f} ± {np.std(delta_AllCells):.2f}, median = {np.median(delta_AllCells):.2f}')
            plt.ylabel('Probability Density')
            plt.title('Histogram and PDF of delta')
            plt.legend()
            if save == True: 
                plt.savefig(f'{parent_dir}{directory}/Histogram of delta.pdf', dpi=300)
            plt.show()
