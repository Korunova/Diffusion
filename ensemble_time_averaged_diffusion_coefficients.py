# -*- coding: utf-8 -*-
"""
Script to estimate diffusion coefficient from time and ensemble averaged mean squared displacement (aeMSD)  
"""
...
taus = [0.01] #minimum lag time for MSD calculation, interpolation
save = False
...
#turn on/off static error remove
remove_static_error = True
error_files = ['D:/082124_082524 folder for not using files in coding/gem FIXED 35Oc/MSD_lagtime5.0ms.csv', 'D:/082124_082524 folder for not using files in coding/GEM fixed 35oC repeat/MSD_lagtime5.0ms repeat.csv', 'D:/082124_082524 folder for not using files in coding/GEM fixed 35oC repeat/MSD_lagtime5.0ms repeat.csv']
...
def FitLinearFunction(name, x, y, label = None, n = None, points=10, R2=0.9, sigma = None, check_plot = False):
    ...
    if check_plot == True:
        ...
        plt.errorbar(x, y, yerr = sigma, fmt = 'o', color = colour, alpha = 1)
        plt.plot(x[0:points], predicted_msd, marker='.', linestyle = '-', linewidth = 2, markersize = 2, color = colour, label = label + f', α = {fitted_a:.2f} ± {a_error:.2f}')
        plt.ylabel(r'log(<MSD>), $\mu$m$^2$', fontsize = 50)
        plt.xlabel('log(τ), sec', fontsize = 50)
        plt.xticks()
        plt.yticks()
...
#loop to try different minimum lag time for relationship MSD ~ t^a
for tau in taus: 

    condition = 0   
    #Estimation of difusion parameters from time and ensemble MSD
    for experiment in experiments: 
        ...
        for idx, repeat in enumerate(experiment): 
            ...
            if remove_static_error:
                ...
                msd_errors = {}
                with open(error_files[idx], mode='r') as file:
            ...
            #collection of MSD anf lag times
            lagtime_data = {}   
        ...
        #visualisation of MSD ~ t^a and estimation of parameters
        ...
        for p in range(5,11):
            ...
            if result[5] < 0.98 or p == 10:
                result = FitLinearFunction(name, time_data_log10[0:10], msd_data_log10[0:10], exp_cond[condition], n = color_number, points = p-1, sigma = msd_std_data_log10[0:10], check_plot = True)
                break
            limit = round(len(msd_data_log10)/10)
    ...
    if save == True: 
        plt.figure(num=13)
        plt.savefig(rf'D:\article_GEM_U2OS_expression\Main Revision\pictures\Figure3_aeMSDrawdata_{day}day_{tau*1000}ms_tracked_{T}T_{day}.pdf', dpi=300)
        plt.savefig(rf'D:\article_GEM_U2OS_expression\Main Revision\pictures\Figure3_aeMSDrawdata_{day}day_{tau*1000}ms_tracked_{T}T_{day}.png', dpi=300)
        plt.show()
        ...
        df.to_csv(rf'D:\article_GEM_U2OS_expression\Main Revision\pictures\Figure3table_aeMSDrawdata_{day}day_{tau*1000}ms_results_tracked_{T}T.csv', index=False)
