...
taus = [0.015] #time displacement for MSD calculation 
r2 = 0.90 # filter of diffusion plots try 0
scale = (1/0.0586000)
...
def FitLinearFunction(name, x, y, points=10, R2=0.9, yerr = None):
    ...
    return name, float(fitted_D), float(D_error), r_squared, x[0:points], predicted_msd
...
for experiment in experiments: 
    ...
    for tau in taus:
        ...
        for min_trajectory_length in min_trajectory_lengths:
            ...
            directory = f'ARTICLE_V3_Brownian_Deff_Dw_{experiment_condition}_R2{r2}_{min_trajectory_length}trlength_5Derr_5aerr_lagtime{tau*1000}ms_nw{nw:.5f}T{T} pdf pictures static'
            ...
            for file in files: 
                ...
                with open(f'{parent_dir}{file}/{experiment_condition}/converted_data_length10.csv') as data:
                    ...
                    for trajectory in trajectories_interpolated:
                        ...
                        if p>0 and result != None and result[3] > r2:
                            
                            #parameters of Brownian motion
                            Deff_value = result[1]/Dw
                            Deff_err = result[2]/Dw
                            
                            if Deff_err*5<=Deff_value: 
                                all_trajectories += 1 
                                ...
                                #plt.xlabel(f'R2 = {result[3]:.2f}, amount of points = {p-1} from {len(time_data_log10)} points')
                                
                                
                                #collect data per one cell
                                Diffusion.append(Deff_value)
                                Diffusion_sigma.append(Deff_err)
                                
                                #collect data  among all cells
                                Diffusion_AllCells.append(Deff_value)
                                Diffusion_Sigma_AllCells.append(Deff_err)
                
                                
                                Tracking_data.append([t, x, y, Deff_value, Deff_err, result[0], time_data_log10, msd_data_log10, msd_std_data_log10]) # [time, x, y, Deff, Deff_sigma, name, tau_log10, msd_log10, msd_std_log10]
                    ...
                    file_data = [file, np.mean(Diffusion), np.std(Diffusion), all_trajectories, confined_trajectories]
                    ...
                    if save == True:
                        ...
                        plt.clf()
                        for trajectory in Tracking_data: # [time, x, y, Deff, Deff_sigma, name, tau_log10, msd_log10, msd_std_log10]
                            data = [trajectory[0], trajectory[1], trajectory[2]]
                            name = f'{trajectory[5]}, {trajectory[3]}, {(trajectory[4])}, Deff and sigma)'
            ...
            plt.xticks()
            plt.yticks()
            plt.xlabel(f'Deff values {np.median(Diffusion_AllCells):.2f} ± {np.std(Diffusion_AllCells):.2f}')
